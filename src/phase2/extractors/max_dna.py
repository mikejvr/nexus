"""
max_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .max (Autodesk 3ds Max binary scene files)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 128 KB (MAX files can be large)
    buf = raw[:128 * 1024]
    size = len(buf)
    ...
    technical: Dict[str, Any] = {
        ...
        # Structural text hints (MAX is binary, but we still compute ratios)
        "is_text_file": False,
        "ascii_ratio": 0.0,
        "unicode_ratio": 0.0,
    ...
    # ------------------------------------------------------------
    # Text structural hints (even though .max is binary)
    # ------------------------------------------------------------
    if raw:
        ...
        # MAX files are binary → unicode_ratio usually high
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("max_dna", path)
