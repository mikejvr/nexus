"""
json_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .json, .jsonc, generic JSON-like text files
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    sigs = {
        b"{": "JSON_OBJECT_HINT",
        b"[": "JSON_ARRAY_HINT",
        b"null": "JSON_NULL_HINT",
        b"true": "JSON_TRUE_HINT",
        b"false": "JSON_FALSE_HINT",
        b"<!--": "XML_COMMENT_HINT",
        b"<?xml": "XML_HINT",
        b"<html": "HTML_HINT",
        b"\x89PNG": "PNG",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("json_dna", path)
