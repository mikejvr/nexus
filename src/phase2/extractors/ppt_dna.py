"""
ppt_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .ppt (legacy PowerPoint OLE2 format)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 64 KB (PPT files can be large)
    buf = raw[:64 * 1024]
    size = len(buf)
    ...
    technical: Dict[str, Any] = {
        ...
        # Structural text hints (PPT is binary, but ratios still computed)
        "is_text_file": False,
        "ascii_ratio": 0.0,
        "unicode_ratio": 0.0,
    ...
    if raw:
        ...
        # PPT is binary → unicode_ratio usually high
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("ppt_dna", path)
