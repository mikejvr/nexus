"""
swf_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .swf (Shockwave Flash)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 64 KB (SWF files can be large)
    buf = raw[:64 * 1024]
    size = len(buf)
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("swf_dna", path)
