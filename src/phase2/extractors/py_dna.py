"""
py_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .py (Python source)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # ------------------------------------------------------------
    sigs = {
        b"#!/usr/bin/env python": "SHEBANG_PY",
        b"#!/usr/bin/python": "SHEBANG_PY_ALT",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("py_dna", path)
