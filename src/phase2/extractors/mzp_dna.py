"""
mzp_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .mzp (3ds Max Plug‑in Package)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # ------------------------------------------------------------
    sigs = {
        b"PK\x03\x04": "ZIP",       # MZP is a ZIP container, but Phase‑1 only detects signature
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
        b"II*\x00": "TIFF_LE",
        b"MM\x00*": "TIFF_BE",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("mzp_dna", path)
