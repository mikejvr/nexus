"""
ts_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .ts, .tsx (TypeScript / TSX)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # ------------------------------------------------------------
    sigs = {
        b"import ": "IMPORT_HINT",
        b"export ": "EXPORT_HINT",
        b"class ": "CLASS_HINT",
        b"function ": "FUNCTION_HINT",
        b"interface ": "INTERFACE_HINT",
        b"type ": "TYPE_HINT",
        b"enum ": "ENUM_HINT",
        b"<": "ANGLE_BRACKET_HINT",   # weak generic/JSX hint
        b"@": "DECORATOR_HINT",
        b"#!/usr/bin/env": "SHEBANG_HINT",
        b"<?xml": "XML_HINT",
        b"<html": "HTML_HINT",
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("ts_dna", path)
