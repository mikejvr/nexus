"""
sld_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .sldprt, .sldasm (SolidWorks part/assembly)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 128 KB (SolidWorks files can be large)
    buf = raw[:128 * 1024]
    size = len(buf)
    ...
    technical: Dict[str, Any] = {
        ...
        # Structural text hints (SLDPRT/SLDASM are binary, but we still compute ratios)
        "is_text_file": False,
        "ascii_ratio": 0.0,
        "unicode_ratio": 0.0,
    ...
    # ------------------------------------------------------------
    sigs = {
        b"SLDPRT": "SLD_PART_HINT",
        b"SLDASM": "SLD_ASSEMBLY_HINT",
        b"SolidWorks": "SLD_SOLIDWORKS_HINT",
        b"SW202": "SLD_VERSION_HINT",
        b"ISO-10303-21": "STEP_HINT",
        b"STEP AP214": "STEP_AP214_HINT",
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
        b"II*\x00": "TIFF_LE",
        b"MM\x00*": "TIFF_BE",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("sld_dna", path)
