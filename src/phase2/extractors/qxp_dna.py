"""
qxp_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .qxp (QuarkXPress Document)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 128 KB for deterministic scanning (QXP files often large)
    buf = raw[:128 * 1024]
    size = len(buf)
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("qxp_dna", path)
