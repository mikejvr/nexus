"""
pub_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .pub (Microsoft Publisher)
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # ------------------------------------------------------------
    sigs = {
        b"\xD0\xCF\x11\xE0": "OLE",        # PUB is OLE2 container
        b"MSPublisher": "PUBLISHER_HINT",  # structural hint only
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
        b"II*\x00": "TIFF_LE",
        b"MM\x00*": "TIFF_BE",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("pub_dna", path)
