"""
svg_dna extractor — Phase‑1 deterministic, stdlib‑only, structural‑only
Supports: .svg
"""
...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 64 KB (SVGs can be large)
    buf = raw[:64 * 1024]
    size = len(buf)
    ...
    sigs = {
        b"<svg": "SVG_HINT",
        b"<?xml": "XML_HINT",
        b"<!DOCTYPE svg": "SVG_DOCTYPE_HINT",
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
        b"II*\x00": "TIFF_LE",
        b"MM\x00*": "TIFF_BE",
    ...
    # ------------------------------------------------------------
    metadata = ext_dna.make_metadata("svg_dna", path)
