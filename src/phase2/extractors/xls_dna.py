...
def extract(ext_dna, path: Path) -> Tuple[bool, Dict[str, Any], Dict[str, Any], List[str]]:
    ...
    # Cap raw to 64 KB for deterministic scanning (XLS files can be large)
    buf = raw[:64 * 1024]
    size = len(buf)
    ...
    sigs = {
        ...
        b"\x09\x08": "BIFF_BOF_HINT",
        b"\x08\x09": "BIFF_BOF_HINT_ALT",
        b"\x0A\x00": "BIFF_EOF_HINT",
        b"Workbook": "WORKBOOK_HINT",
        b"Book": "BOOK_HINT",
        b"\x89PNG": "PNG",
        b"\xFF\xD8\xFF": "JPEG",
        b"II*\x00": "TIFF_LE",
